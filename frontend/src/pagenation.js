...
function PaginationComponent({ user }) {  // . user 프로퍼티를 props로 받습니다. (세션에 저장중인 유저정보를 얻기위해)
    ...
    const fetchPosts = async (pageNumber) => { // 페이지 번호에 해당하는 게시글을 가져오는 함수를 정의합니다.
        try {
            const response = await axios.get(`http://52.65.217.35:8081/pagenation?page=${pageNumber}`); // 페이지 번호에 따라 게시글을 가져옵니다.
    ...
    const handleDeletePost = async (post_id, author_name) => {
        if (user.name === author_name) {
            try {
                await axios.delete(`http://52.65.217.35:8081/post/${post_id}`, {
                    headers: {
    ...
    const fetchSearchedPosts = async (pageNumber) => {  // 실시간 검색, 
        try {
            const response = await axios.get(`http://52.65.217.35:8081/post/search?term=${searchTerm}`);
